...
**网络架构**（纯 CPU 实现）：
- Actor: State(5) → FC(64) → ReLU → FC(64) → ReLU → Action(2)
- Critic: State(5) → FC(64) → ReLU → FC(64) → ReLU → Value(1)

**训练参数**：
- γ=0.99, λ=0.95, ε=0.2, lr=3e-4
- 500 episodes, batch size=32, epochs=10
...
**解释内容**：
1. **决策依据**：电价时段、SOC 状态、新能源出力
2. **决策逻辑**：为什么选择充/放电，预期收益
3. **优化目标**：成本、消纳率、碳排放
...
| 项目 | 配置 |
|------|------|
| 运行环境 | Python 3.x（纯 CPU） |
| 实验类型 | 软件模拟（无真实 GPU 硬件） |
| 计算方式 | 基于数据集参数的数值模拟 |
| 主要库 | NumPy, CSV, JSON |

**注**：实验通过软件模拟数据中心功耗行为，使用数据集中的 GPU 数量（10,412 卡）和功耗参数（静态 595.6 kW，满载 3,112.4 kW）进行计算，但未使用真实 GPU 硬件。

**GPU 集群功耗参数**（基于真实数据统计，用于模拟）：

| 参数 | 值 | 说明 |
|------|-----|------|
| GPU 总数 | 10,412 卡 | A100/A10/H800 等混合 |
| 静态功耗 | 595.6 kW | 所有 GPU 空闲功耗总和 |
| 满载功耗 | 3,112.4 kW | 所有 GPU 满载功耗总和 |
| 平均单卡静态 | 57.2 W | 595.6 kW / 10,412 卡 |
| 平均单卡满载 | 298.9 W | 3,112.4 kW / 10,412 卡 |
...
| 新能源出力 | 0.13-0.26 MW | 24 小时变化（风电 + 光伏） |
| 数据中心负载 | 0.60-1.38 MW | 基于 GPU 静态功耗×(1+ 利用率) |
| 储能容量 | 5 MWh | 与负载匹配 |
| 充放电功率 | 1 MW | 合理比例 |
...
- $f_{\text{hour}}(t)$：时段因子（峰段 1.5，晚间 1.2，谷段 0.8）
...
- 负载：0.60-1.38 MW（基于 GPU 功耗）
- 负载/新能源比：2.3-5.3（负载持续大于新能源）

### 6.4 LLM 可解释性调度实验设计

**实验配置**：
- LLM 模型：google/gemma-3-4b（4B 参数）
- 部署方式：Lmstudio 本地部署
- API 端点：http://localhost:1234/v1/chat/completions
- 调用方式：HTTP POST，JSON 格式
...
**注**：LLM 实验需要本地部署 Lmstudio 并加载 google/gemma-3-4b 模型。如 LLM 不可用，系统使用基于规则的回退解释。
...
1. **新能源消纳率 100%**：
   - 原因：负载（0.60-1.38 MW）持续大于新能源出力（0.13-0.26 MW）
   - 所有可用新能源都被数据中心消纳
   - 无需弃风弃光
...
1. **场景规模**：实验基于单数据中心，负载较小（0.6-1.4 MW）
2. **新能源比例**：数据中心消纳新能源比例较低（0.03%）
3. **PPO 优势**：在小规模场景下，与简单方法性能接近
4. **LLM 依赖**：需要本地部署 Lmstudio，增加系统复杂度

**未来改进**：

1. **扩展场景**：多数据中心协同，更大规模负载
2. **提高新能源比例**：研究负载<新能源场景下的优化策略
3. **PPO 优化**：调整奖励函数，提升消纳率优化效果
4. **LLM 集成**：实现完整的可解释性调度系统
