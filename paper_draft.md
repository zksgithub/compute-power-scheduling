...
**分析**：
- **PPO 方法**通过深度强化学习，在成本（-23.8%）和碳排放（-28.3%）上表现最优
- PPO 消纳率较低（66.1%），因为奖励函数侧重成本/碳排优化
- **MPC 方法**通过滚动预测优化，消纳率最高（78.1%）
- **Greedy 方法**简单有效，三项指标均优于 Rule-based
- 负载（210-458 MW）与新能源出力（293-571 MW）匹配度较高

**图 1：24 小时调度曲线**（新能源、负载、储能 SOC）

### 6.4 储能系统影响

**表 2：储能容量影响**（Greedy vs 无储能）

| 配置 | 成本 | 消纳率 | 碳排放 |
|------|------|--------|--------|
| 无储能 (Rule-based) | 164.7 | 76.8% | 0.0870 |
| 20 MW/100 MWh (Greedy) | 157.7 | 77.6% | 0.0837 |
| 提升 | -4.3% | +0.8% | -3.9% |

### 6.5 讨论

**实验观察**：
1. **PPO 在成本和碳排放上最优**（-23.8%/-28.3%），但消纳率较低
2. **MPC 消纳率最高**（78.1%），但成本略高（+7.9%）
